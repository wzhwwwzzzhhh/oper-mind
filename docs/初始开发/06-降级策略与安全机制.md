...
> **注意：`test_fallback()` 是验证规则引擎的临时测试代码。**
> 验证完规则能正确匹配后，建议直接删掉这个函数。
> 原因：
> 
> 1. 它的 5 条硬编码 SQL 只适合一次性验证，留着没有实际用途
> 2. `--fallback` 参数如果还想保留，可以改为交互式输入（见下方示例），但这不是必须的
> 3. 真正的测试应该写在 `tests/` 目录下，而不是放在 `main.py` 里
> 
> 如果你想把 `--fallback` 改成交互模式：
> 
> ```python
> if __name__ == "__main__":
>     import sys
...
- [x] `python src/main.py --fallback` 能输出 5 条 SQL 的规则诊断结果
- [x] 规则引擎能正确识别含 `status` 的 SQL 并给出加索引建议
- [x] 规则引擎能正确识别 `ORDER BY` 并给出加索引建议
- [x] 规则引擎能正确识别 `JOIN` 并给出加索引建议
