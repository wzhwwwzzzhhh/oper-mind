...
以下 A–E 整体仍只是依赖建议，不是路线图承诺、阶段编号或 Workpack。首个零行为变化候选包已在本文件确认后的独立决策中进入正式路线；该决定不扩展到 A 的其余内容或 B–E。每次实际工作仍须只选择 1–3 个紧密切片，并单独完成所需 Design → Review → 用户确认。
...
| 变化 | 必须先完成 |
|---|---|
| P9 首个候选包进入正式路线图 | 已完成：登记在当前完善收口后、第三阶段扩展前；只包含已确认 PRD，不包含完整 A–E |
...
用户已确认本文，P9 研究阶段据此收口。此后又经独立决策，将首个零行为变化候选包登记为当前完善收口后、第三阶段扩展前的基础工程段。当前边界是：

- 正式路线只承诺已确认 PRD 的三个切片，不承诺完整 A–E；
- 不创建 Workpack；
- 不修改 Runtime、数据库、API 或生产行为；
- A 的其余内容与 B–E 仍只能作为未来决策输入。

是否创建 issue、何时开始实施 Design，以及未来是否继续 A 的其余内容或 B–E，仍须逐项另行决定，不能由本文件自动触发。
