...
| **M7.1** | 同步诊断闭环 | 输入问题 → `POST /diagnose` → loading / 报告 / 复制 / 错误态；不引入 SSE | ⏳ 待开始 |
| **M7.2** | Trace 回放与三路拓扑 | 用真实同步 trace + 固定 fixture 表现 direct / chain / parallel；不推断质量节点 | ⏳ 待开始 |
...
- 每一步完成后先写对应 `stepN-*.md` 快照，再进入独立 Review。
