...
**P3.4b 已完成代码、自动验证和独立审查；当前等待 P3.4c 代码授权。**b 已将 Result 面板接入选定 Run，完整成功 Result 才展示；failed/cancelled/queued/running、载荷矛盾、归档历史和读取错误均按公开契约收口。P3.4c 才补齐 MSW/独立 Mock 的完整 Result 合同并执行 8100→5175/人工验收；不接入真实资源、Trace 或报告能力。c.1 仍使用独立 8100 mock 与单独 5175 前端验收实例，默认开发代理仍指向 8000；c.2 不创建、编辑、实时消费或连接任何真实资源。

P3.2/P3.3 的会话恢复、Run 受理和持久化事件/SSE 已完成；P3.4a 已用完整静态契约夹具完成 Result reader/摘要面板，P3.4b 已接入选定 Run 并通过 37 项前端回归；P3.4c 才补齐独立 Mock Result 并做 8100→5175 验收。当前 P3.3c Mock 的 Result 缺 P2 必填 `created_at`，不能作为 P3.4 结果验收事实；不得以真实 8000/数据库替代。用户选择将真实 API/数据库验收延后，届时仍必须由用户/数据库所有者确认 C1–C8（目标、受控 URL、只读权限、revision、安全数据、实例、契约、回退）。**当前唯一下一步为 P3.4c：补齐完整结构化 Result 的 MSW/独立 Mock FastAPI 契约，并完成独立代理与人工验收（需用户后续代码授权）。**
