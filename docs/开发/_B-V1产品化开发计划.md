...
**P3.4 Design 与独立审查已完成；当前等待 P3.4a 代码授权。**P3.4 只消费 `GET /api/v1/runs/{run_id}` 的嵌入式 `DiagnosisResult`，定义成功/失败/取消/非终态、空数组、归档只读与后续受控 Trace 条件；不新增后端接口或 `report/` 入口。c.1 仍使用独立 8100 mock 与单独 5175 前端验收实例，默认开发代理仍指向 8000；c.2 不创建、编辑、实时消费或连接任何真实资源。

P3.2/P3.3 的会话恢复、Run 受理和持久化事件/SSE 已完成；P3.4a 先以完整静态契约夹具实现 Result reader/摘要面板，P3.4b 再收口终态与归档，P3.4c 才补齐独立 Mock Result 并做 8100→5175 验收。当前 P3.3c Mock 的 Result 缺 P2 必填 `created_at`，不能作为 P3.4 结果验收事实；不得以真实 8000/数据库替代。用户选择将真实 API/数据库验收延后，届时仍必须由用户/数据库所有者确认 C1–C8（目标、受控 URL、只读权限、revision、安全数据、实例、契约、回退）。**当前唯一下一步为 P3.4a：结构化结果读取模型与摘要面板实现（需用户后续代码授权）。**
