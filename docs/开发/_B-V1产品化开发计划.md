...
**P3.3b 持久化事件与 SSE 恢复已提交为 `e7858ce`；P3.3c 的代码、自动测试、独立 8100 mock + 5175 Vite 代理及用户可视化主流程验收均已通过，且独立 Review 通过；当前等待提交授权。**c.1 使用独立 8100 mock 与单独 5175 前端验收实例，默认开发代理仍指向 8000；c.2 不创建、编辑、实时消费或连接任何真实资源。

P3.2 不创建 Session/Run、不接入 SSE、不实现结构化结果，不访问旧 `/diagnose` 或 `/diagnose/stream`。c.1 的 FastAPI mock 与 MSW 分开验收：根入口、深链回填、刷新、404、500、归档、cursor、跨 Session 保护已通过；Vite 代理上游中断时前端安全显示非 JSON 协议错误，不降级为假数据。c.2 已离线确认 Alembic head、URL 优先级、启动不自动迁移与 mock 回退边界；用户选择将真实 API/数据库验收延后，届时仍必须由用户/数据库所有者确认 C1–C8（目标、受控 URL、只读权限、revision、安全数据、实例、契约、回退）。**当前唯一下一步为等待用户提交 P3.3c；提交后进入 P3.4 Design：结构化结果、失败/空/归档收口与受控 Trace 入口。**
