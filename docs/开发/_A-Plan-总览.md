...
| **P3** | **主前端工作台** | **🟡 P3.3 Design** | P3.2 已完成 mock/离线验证；真实数据库只读验收按用户决策延后，当前设计 Run 受理、幂等与 SSE 恢复 |
...
**当前唯一下一步：P3.3 Design：Run 受理、幂等与 SSE 恢复。**真实数据库只读验收延后；届时仍须先确认 C1–C8。P3.3 Design 不连接真实 DB 或数据源、不运行在线 Alembic、不修改 8000 后端，也不混入完整结果卡、Trace 跳转或 P4/P5/P6 资源。
## 5. 执行与降级原则
