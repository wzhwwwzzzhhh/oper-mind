...
本 Step 仅同步产品方向、真实目录、阶段真相源、前端职责、日志/交接规则和历史基线提示；不修改业务代码，不初始化主前端工程，不删除 `report/`。完成独立 Review 并得到提交授权前，P0.1 仍是当前唯一下一步。

P0.1 提交后，唯一下一步切换为 **P0.2：现状盘点与产品架构**，输出后端现状到 V1 契约的差距、应用层/Agent Core/基础设施边界、核心实体关系图与状态机；不进行无业务收益的大搬家。
