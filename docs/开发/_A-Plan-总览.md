...
| **P3** | **主前端工作台** | **🟡 P3.4a 待授权** | P3.4 Design 与独立审查已完成；下一步是结构化结果读取模型与摘要面板实现，真实数据库只读验收继续延后 |
...
**P3.4 Design 已完成并独立审查通过：Result 只消费 `GET /api/v1/runs/{run_id}` 的嵌入式结构化资源；失败/空/归档、刷新/SSE 与 `report/`/Trace 边界均已定义。当前唯一下一步为 P3.4a：结构化结果读取模型与摘要面板实现（需用户后续代码授权）。**
