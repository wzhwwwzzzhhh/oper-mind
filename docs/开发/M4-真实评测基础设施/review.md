...
人工抽检准备阶段发现短期记忆会在 `run_suite()` 的相邻用例之间残留。现已新增评测专用重置接口：
Runner 每条用例前清空所有领域 Agent 的短期会话、思考记录与 Coordinator trace，而长期记忆仍保持关闭。
定向回归 `18 passed in 5.35s`；日常 CLI / API 的多轮短期记忆不受影响。

---

# M4 收尾说明（2026-07-20 · A-Plan 目标调整）

**状态：✅ 完成（作为「真实评测基础设施与条件切换」里程碑）。**

M4 交付的是真实评测的**基础设施**——双模型配置、裁判口径校准、6 组实验条件切换、可比指标——均已完成并通过测试（`55 passed`）。原命名「真实模型**实验**」承诺了「跑完实验」，与实际交付（**能跑实验的基建**）不符，故里程碑重命名为 `M4-真实评测基础设施`，避免"半成品"错觉。

原「剩余门槛」按 A-Plan（全栈 + agent 求职竞争力）目标重新裁决：

- **12 条 κ 人工抽检**：属论文级严谨，A-Plan 下**降级/推迟**，不阻塞后续。
- **single_agent vs full 正式对比 + 出曲线**：属 A-Plan 承重墙，**不砍**；但需先补「区分度数据集」才有统计意义，故**并入 M6-多Agent价值对比**执行。

后续路线以 `docs/开发/_A-Plan-总览.md` 为唯一真相源。
