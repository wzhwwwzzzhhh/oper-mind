...
| AC6 | `DiagnosisRunData.service_id`、`SqlAlchemyDiagnosisRunRepository.add`、`RunApplicationService`、Run 资源 | PASS | 服务端从 Session 读取并持久化，客户端不能覆盖 |
...
| AC10 | 干净交付树后端 `106 passed`；此前前端 `53 passed`、typecheck、build | PASS | 相关回归全绿 |
| AC11 | `SERVICE_CONTEXT_REQUIRED` 受理守卫；`ShowIndexTool(service_id=None)` 返回“数据库未选择目标服务” | PASS | 未绑定 DB 调查不创建 Run、不连接固定生产库 |
