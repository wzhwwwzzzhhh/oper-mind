...
- 服务端以已声明的 exact health default query 作为内部 profile 选择器：Graph 在模型路由前固定 `direct/db`，DBAgent 仅向模型和 ToolGateway 暴露 `check_connection_pool`，并将本次健康调查 Tool 接纳上限固定为 1；编辑后的普通调查仍走既有 PostgreSQL 受限 Tool 菜单。
...
- 五个固定 MySQL 指标必须精确齐全；任一缺失时 Connector 快照直接为 `unavailable/malformed_fact`，服务注册的 `test_connection()` 同步返回 unavailable，不再出现服务中心健康而 Agent 拒绝的分叉。
...
- P12 preflight 只验证 opt-in/目标/credential ref；独立 Runner 在 TTY 二次确认后按 kind 选择精确 health Tool，并在成功退出前验证唯一终态、唯一成功 Tool 事件与 Result。离线测试通过注入临时 SQLite runtime 与 fake connector，实际覆盖 Registry 装载、origin match/mismatch、连接测试、Session、Run、Trace/Result；Runner 的真实入口未执行。
...
- **AC4 自动化满足**：正式 PG E2E 形成唯一 succeeded、一个成功 health Tool Trace 和 Result；exact health query 在模型路由前锁定 direct/db，模型可见/Gateway 可执行菜单只有 `check_connection_pool`，重复 Tool call 探针证明底层最多执行一次；既有慢查询 Tool 保留受限参数/只读能力。
...
- **AC8 自动化满足**：MySQL DSN、短生命周期、超时、未配置/不可达/权限/cleanup/malformed 探针通过；缺任一固定指标时 Connector 与公开连接测试均 unavailable。
...
- **AC14 待真实验收**：独立 Runner 与安全验证逻辑已实现；完整离线编排测试已覆盖 Registry/origin/connection/Session/Run/Trace/Result 及 origin mismatch 前置停止。本机 MySQL、远端非生产 PostgreSQL、远端非生产 Redis 均未获得逐目标当次访问授权，故未执行；真实模型也未执行。
...
- 后端全量返修后：`866 passed, 6 warnings`（`377.41s`）；除沙箱无法写 `.pytest_cache` 外均为既有依赖弃用告警，无测试失败。
