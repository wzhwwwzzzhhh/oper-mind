...
- **worktree 是全新 checkout**：仓库根的 `.venv`、`node_modules` 不会带过去，需在 worktree 内重建（后端 venv、前端 `npm install`）。
  - **环境预热脚本（推荐）**：新 worktree 建好后，在其根目录执行 `.Codex/scripts/init-worktree.ps1`（Windows）或 `.Codex/scripts/init-worktree.sh`（Unix），一把梭建 venv、设 UTF-8、装依赖，消除重复踩坑。详见 `.Codex/scripts/README.md`。
  - **环境已知坑（Windows）**：
