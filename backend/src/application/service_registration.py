...
ALLOWED_KINDS = frozenset({"postgres", "redis"})
INSTANCE_ID_PATTERN = re.compile(r"^[a-z0-9][a-z0-9._-]*$")

ConnectorFactory = Callable[[str, str | None, str, str, str | None], ServiceConnector]
...
class ServiceRegistrationApplicationService:
    ...
    def update(self, command: UpdateServiceCommand) -> ServiceRegistrationData:
        ...
        updated = _in_transaction(self._session_factory, operation)
        current = self._registry.get_connector(command.instance_id)
        if current is None:
            raise ServiceRegistrationNotFoundError()
        encrypted = updated.dsn_encrypted
        nonce = updated.dsn_nonce
        resolved_dsn = self._resolve_dsn(encrypted, nonce)
...
def _default_connector_factory(
    ...
    return build_service_connector(kind, dsn, instance_id, title, masked_tail)
