...
存储经 domain 的 ``AppSettingsStore`` 端口注入，具体实现在 api 层装配。
"""
...
class ModelParamsApplicationService:
    ...
    def get(self) -> ModelParams:
        ...
        try:
            raw = self._store.get(MODEL_PARAMS_KEY)
    ...
    def set(self, params: ModelParams) -> None:
        ...
        try:
            self._store.set(MODEL_PARAMS_KEY, encode_params(params))
        except SQLAlchemyError as error:
            raise ModelParamsPersistenceError() from error


def resolve_model_params(store: AppSettingsStore) -> ModelParamsResolution:
    ...
    """
    try:
        raw = store.get(MODEL_PARAMS_KEY)
    except SQLAlchemyError:
