...
def upgrade() -> None:
    ...
    with op.batch_alter_table("run_events", schema=None) as batch_op:
        batch_op.drop_constraint("run_event_type_valid", type_="check")
        batch_op.create_check_constraint("run_event_type_valid", _TYPES_WITH_TOOL)
...
def downgrade() -> None:
    ...
    with op.batch_alter_table("run_events", schema=None) as batch_op:
        batch_op.drop_constraint("run_event_type_valid", type_="check")
        batch_op.create_check_constraint("run_event_type_valid", _TYPES_ORIGINAL)
