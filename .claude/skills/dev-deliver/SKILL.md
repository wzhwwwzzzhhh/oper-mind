...
8. **清理 worktree（PR 合并后必须）**：`git worktree remove "D:/market-handsome/oper-mind-worktrees/<切片>"` + `git branch -d <类型>/<切片>`，
...
- 关联 GitHub issue 已关闭（`gh issue close <编号>`）
...
6. **issue 状态一致**：交付完成必须关闭关联 issue（`gh issue close <编号>`），PRD 状态与 issue 状态一致，不留"PRD 完成但 issue 仍 open"。
...
- 交付完成未关闭关联 issue，或 issue 状态与 PRD 状态不一致（PRD 完成但 issue 仍 open）
