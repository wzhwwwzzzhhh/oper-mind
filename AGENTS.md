...
- P4.0 Work 1 已在用户授权靶场真实 smoke 通过。P4.1 Design/Review 已完成，等待用户明确实施授权；P4.1 只做会话触发的只读慢查询调查与最小证据展示。
...
- 只有独立受控执行器才能在用户授权隔离靶场、人工审批、严格 action 白名单同时满足时执行预定义动作；P4.2 前不得实现写入动作。
